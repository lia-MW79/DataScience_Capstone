...
def get_pie_chart(entered_site):
    if entered_site == "ALL":
        ...
        fig = px.pie(filtered_df, values="Launch Site",
            names="class", 
            title="Successful and Failed Launches for Site {}".format(entered_site))
        return fig
...
def get_scatter_chart(entered_site, payload_range):
    if entered_site == "ALL":
        all_df=spacex_df[(spacex_df["Payload Mass (kg)"] >= payload_range[0])
        ...
        # return the outcomes piechart for a selected site
        filtered_df = spacex_df[(spacex_df["Launch Site"]==entered_site) & (spacex_df["Payload Mass (kg)"] >= payload_range[0])
            & (spacex_df["Payload Mass (kg)"] <= payload_range[1])]
